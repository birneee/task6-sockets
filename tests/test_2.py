...
def main() -> None:
    ...
    with open("client_output_test_2.txt", "w+") as stdout:
        try:
            ...
            with subprocess.Popen([cmd, "1025"], stdout=subprocess.PIPE, env=env) as proc:
                info("Run single-threaded client test (1 thread) ...")
                run_project_executable("client_test", args=["localhost", "1025", "10000"], stdout=stdout, extra_env=env)
                proc.kill();
